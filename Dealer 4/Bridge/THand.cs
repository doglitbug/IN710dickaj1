        ...
        public void printHand(ListBox box)
        {
            ...
            for (int i = 0; i < handArray.Count(); i++)
            {
                ...
                switch(handArray[i].Rank+2){
                    ...
                    //If not a picture card(or Ace) its is a number with a space behind it)
                    default:
                        ourCardCode = (handArray[i].Rank+2).ToString() + " ";
                        break;
                }

                //Add the card to the list under the correct suit list
                tempStrings[handArray[i].Suit] += ourCardCode;
            }
            ...
            for (int i = 3; i >= 0; i--)
            {
                box.Items.Add(tempStrings[i]);
            }

            //Add HCP and player seperator to box
            box.Items.Add("HCP: " + getHCP());
            box.Items.Add(" ----------------- ");
